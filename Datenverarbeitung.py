            ...
            for index, row in ergebnisse_xml.iterrows():
                ...
                if ne_element is not None:
                    ...
                    for i, (coord, original_coord) in enumerate(zip(interpolated_coords, interpolated_original_coords)):
                        ...
                        data_list = []  # Eine Liste, um die Daten zu speichern
                        ...
                        data_list.append(dictionary)  # Füge das Dictionary zu der Datenliste hinzu
                        Tabelle = pd.concat([Tabelle, pd.DataFrame(data_list)], ignore_index=True)  # Verknüpfe den DataFrame mit den neuen Daten
                    counter+=1
